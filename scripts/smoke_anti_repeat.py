...
Critério de aprovação: para cada par (nova bolha, bolha anterior do leonardo),
...
async def main() -> int:
    ...
    history: list[dict] = [msg("outbound", "Olá! 👋 Bem-vindo à Castro Veículos. Vi que você demonstrou interesse no Chevrolet Montana 🚗. Posso te passar mais informações sobre ele?")]
    all_leonardo_bubbles: list[str] = []
    fail_count = 0
    ...
    for label, state, update, last_msg, tools in SCENARIOS:
        ...
        for new_b in bubbles:
            n = normalize(new_b)
            for old_b in all_leonardo_bubbles:
        ...
        all_leonardo_bubbles.extend(bubbles)
